...
def cards():
    '''
cards()=displays a card of the diamond suit
@param deck=object of class PlayingCard
returns deck now holding diamond suit card
PlayingCard class at the bottom
    '''
    try:
        deck=pCard.PlayingCard()
        while True:
            deck.selectAtRandom()
            if deck.getSuit()=="diamonds":
                break
        return deck
    except:
