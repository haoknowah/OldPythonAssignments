...
than one of them in change since 2 nickels = 1 dime
    ...
    try:
        ...
        q.set(str((x-(int(sackEntry.get())*100))//25))
        quartersEntry["textvariable"]=q
        print("A")
        d.set(str((x-(int(sackEntry.get())*100)-(int(quartersEntry.get())*25))//10))
        dimesEntry["textvariable"]=d
        n.set(str((x-(int(sackEntry.get())*100)-(int(quartersEntry.get())*25)-(int(dimesEntry.get())*10))//5))
